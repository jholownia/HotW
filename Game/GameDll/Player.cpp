...
void CPlayer::Update(SEntityUpdateContext& ctx, int updateSlot)
{
	...
	// (jh) Here the input system is updated or if it doesn't exist yet - created.
	if (m_pPlayerInput.get())
...
void CPlayer::UpdateView(SViewParams &viewParams)
{
	...
	// (jh) this is where we get a separated camera from the CameraManager
	// The CPlayerView class is not used here
	...
	*/}
	...
	// (jh) here the HUD gets rendered/updated
...
void CPlayer::PostUpdateView(SViewParams &viewParams)
{
	...
	/*
		...
		viewParams.rotation.SetRotationX(-gf_PIHalf*0.75f);
		viewParams.position.z += 20.0f;
	...
	{
		CryLog("x < left");
		offsetX -= 5.0 * renderer->GetGPUFrameTime();
	}
	...
	{
		CryLog("y > top");
		offsetY -= 5.0 * renderer->GetGPUFrameTime();
	}
...
// (jh) Another important function called from init
