...
//------------------------------------------------------------------------
// (jh) This function sets some important stuff on map loading
bool CGameRules::Init( IGameObject * pGameObject )
{
	...
	{
		// (jh) Here we get client and server side (and collision) functions from tables from the Lua script
...
//------------------------------------------------------------------------
// (jh) This is probably where the player is spawned.
// Thing to note is that most of the stuff ( e.g. spawn point) is actually set in 
// GameRules/SinglePlayer.lua script called from this function.
...
{ 			
	...
	// (jh) set env variables here
	// Set environment variables to use the new camera	
	{
		ICVar* consoleVar = gEnv->pConsole->GetCVar("cl_cam_orbit");
		consoleVar->Set(1);

		consoleVar = gEnv->pConsole->GetCVar("g_tpview_control");
		consoleVar->Set(1);

		consoleVar = gEnv->pConsole->GetCVar("g_tpview_enable");
		consoleVar->Set(1);		
	}
